...
"""
Assigns students based on smallest stress values. If stress levels exceeded, removes
...
def greedy_solve_3(G, s):
    ...
    if s_level exceeded, remove students that cause most stress, open 2 room, repeat
     - Remember to sort students (or sort edgelist or somt) and break ties by stress
    ...
    stress_edgeList = sorted(G_copy.edges, key=lambda x: G_copy.edges[x]["stress"])
    ...
    stress_dict = {}
    for edge in stress_edgeList:
        #print("edge: ", edge)
        #print("happiness: ", G_copy.edges[edge]["happiness"])
        if G_copy.edges[edge]["stress"] in stress_dict:
            stress_dict[G_copy.edges[edge]["stress"]] += [edge]
        else:
            stress_dict[G_copy.edges[edge]["stress"]] = []
            stress_dict[G_copy.edges[edge]["stress"]] += [edge]

    while (len(assigned_students) < len(list(G.nodes))):

        #Todo: does this line actually work
        stress_edgeList = remove_students_greedy_stress(G, G_copy, s, room_to_students, stress_dict, assigned_students, stress_edgeList)
        stress_edgeList = sorted(stress_edgeList, key=lambda x: G_copy.edges[x]["stress"])
        ...
        while (len(assigned_students) < len(list(G.nodes))):
            ...
            pop_amt = min(len(stress_edgeList), 5)
            if (pop_amt <= 0):
                break
            #print("assigned_students: ", assigned_students)
            #print("room_to_students: ", room_to_students)
            #print("happy_edgeList: ", happy_edgeList)
            student_pair = stress_edgeList.pop(random.randint(0, pop_amt-1)) # ToDo: Maybe increase this value to 5 - 10?
            ...
            if (student0_assigned):
                ...
                for room in room_to_students:
                    ...
                    if (temp_happy1 > max_happy1 and valid1):
                        max_happy1 = temp_happy1
                        max_room1 = room

                    room_to_students[room].remove(student1)

                if (max_room1 >= 0):
                    room_to_students[max_room1] += [student1]
                    assigned_students += [student1]
                else:
            ...
            if (student1_assigned):
                ...
                max_happy0 = float('-inf')
                max_room0 = -1
                for room in room_to_students:
                    room_to_students[room] += [student0]

                    #check if adding students to this room causes minimum stress (disruption?)
                    temp_happys0 = utils.calculate_happiness_for_room(room_to_students[room], G)
                    #check if solution is valid when adding students to this room
                    valid0 = utils.is_valid_solution(utils.convert_dictionary(room_to_students), G, s, i)
                    if (temp_happy0 > max_happy0 and valid0):
                        max_happy0 = temp_happy0
                        max_room0 = room

                    room_to_students[room].remove(student0)

                if (max_room0 >= 0):
                    room_to_students[max_room0] += [student0]
                    assigned_students += [student0]
                else:
                # at this point, student1 cant be assigned to any room without causing excess stress,
                # so this solution/number of breakout rooms cannot work, so try opening more rooms
            ...
            if (not student1_assigned and not student0_assigned):
                ...
                for room in room_to_students:
                    ...
                    #check if adding students to this room causes minimum stress (disruption?)
                    temp_happy = utils.calculate_happiness_for_room(room_to_students[room], G)
                    #check if solution is valid when adding students to this room
                    valid = utils.is_valid_solution(utils.convert_dictionary(room_to_students), G, s, i)
                    if (temp_happy < max_happy and valid):
                ...
                if (max_room >= 0):
                    room_to_students[max_room] += [student0]
                    room_to_students[max_room] += [student1]
                    assigned_students += [student0]
                    assigned_students += [student1]
                    continue
...
def remove_students_greedy_stress(G, G_copy, s, room_to_students, stress_dict, assigned_students, stress_edgeList):
