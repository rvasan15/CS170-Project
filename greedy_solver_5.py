...
"""
Assigns students based on largest (alpha * happy - beta * stress). If stress levels exceeded,
removes students that cause most stress in each room and adds them back to list of unassigned
...
def greedy_solve_5(G, s, alpha, beta, pop_limit=5):
    ...
    value_edgeList = sorted(G_copy.edges, key=lambda x: alpha * G_copy.edges[x]["happiness"] - beta * G_copy.edges[x]["stress"], reverse = True)
    ...
    for edge in value_edgeList:
        ...
        val = alpha * G_copy.edges[edge]["happiness"] - beta * G_copy.edges[edge]["stress"]
    ...
    while (len(assigned_students) < len(list(G.nodes))):
        ...
        while (len(assigned_students) < len(list(G.nodes))):
            ...
            student_pair = value_edgeList.pop(random.randint(0, pop_amt-1)) # ToDo: Maybe increase this value to 5 - 10?
            ...
            if (student0_assigned):
                ...
                # so this solution/number of breakout rooms cannot work, so try opening more rooms
                    ...
                    #break
                    value_edgeList = remove_students_greedy(G, G_copy, s, room_to_students, value_dict, assigned_students, value_edgeList, alpha, beta)
                    continue
...
def remove_students_greedy(G, G_copy, s, room_to_students, value_dict, assigned_students, value_edgeList, alpha, beta):
    ...
    while True:
        ...
        for i in range(len(removed_students)-1):
            for j in range(i+1, len(removed_students)):
                ...
                #print(edge)
                value_edgeList += [edge]
                val = alpha * G_copy.edges[edge]["happiness"] - beta * G_copy.edges[edge]["happiness"]
                if val in value_dict:
                    value_dict[val] += [edge]
                else:
                    value_dict[val] = []
        ...
        value_edgeList = sorted(value_edgeList, key=lambda x: alpha * G_copy.edges[x]["happiness"] - beta * G_copy.edges[x]["happiness"], reverse = True)
